...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        #     # compute all distances from p -> each points in other
        #     distances.append([p.distance(o) for o in other.members()])
        #
        # return min([min(d) for d in distances])
        for p in self.points:
        ...
        for i in range(len(self.members) - 1):
            ...
            while j < len(self.members):
                ...
                if shortest > temp:
                    shortest = temp
                    # catch the tuple
                    catch = (self.members[i], self.members[j])
