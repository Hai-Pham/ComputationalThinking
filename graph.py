...
class WeightedDigraph(Digraph):
    ...
    def addEdge(self,edge):
        ...
        if not (source in self.nodes and dest in self.nodes):
            raise ValueError("Node not in WeightedDigraph")
        else:
            self.edges[source].append([dest, (dist, outDist)])

    # helper function to calculate the distance between 2 nodes
    def totalDistance(self, start, end):
        # must loop 2 times to pick out the needed distance
        for node in self.edges[start]:
            if node[0] == end:
                t = node[1][0]
        return float(t)
    def outdoorDistance(self, start, end):
        for node in self.edges[start]:
            if node[0] == end:
                o = node[1][1]
        return float(o)
